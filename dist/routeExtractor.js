...
export async function extractRoutesFromServer({ serverFilePath, collectionName, }) {
    ...
    try {
        ...
        if (stack) {
            ...
            allRoutes.forEach((route) => console.log(`${route.methods.join(", ")} ${route.path}`));
            const collection = generatePostmanCollection(allRoutes, collectionName);
            const collectionPath = path.join(process.cwd(), `${collectionName || "postman-collection"}.json`);
